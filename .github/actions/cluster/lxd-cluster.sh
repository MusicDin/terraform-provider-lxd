...
VERSION_MICROCEPH="${VERSION_MICROCEPH:-latest/stable}"
...
configure_microceph() {
        ...
        lxc exec "${microceph}" --env=DEBIAN_FRONTEND=noninteractive -- apt-get -qq -y install snapd
        ...
        lxc exec "${microceph}" --env=DEBIAN_FRONTEND=noninteractive -- apt-get -qq -y install ceph-common

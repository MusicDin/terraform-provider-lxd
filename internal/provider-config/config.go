...
func NewLxdProviderConfig(version string, remotes map[string]LxdRemote, options Options) (*LxdProviderConfig, error) {
	...
	if envRemoteName != "" {
		...
		envAddress := os.Getenv("LXD_ADDR")
		envScheme := os.Getenv("LXD_SCHEME") // Deprecated!
		envPort := os.Getenv("LXD_PORT")     // Deprecated!

		// Construct LXD address from environment variables.
		address, err := DetermineLXDAddress(protocol, envScheme, envAddress, envPort)
		if err != nil {
...
func (p *LxdProviderConfig) setRemote(remoteName string, remote LxdRemote) error {
	...
	if remote.Password != "" && remote.Token != "" {
		return fmt.Errorf("Remote token and password are mutually exclusive")
	}
...
func (p *LxdProviderConfig) acceptRemoteCertificate(remoteName string, url string) error {
	...
	if !p.acceptServerCertificate {
		return fmt.Errorf("Unable to communicate with remote server. " +
...
func DetermineLXDAddress(protocol string, scheme string, address string, port string) (string, error) {
	// Try to extract scheme from the address.
	if strings.Contains(address, "://") {
		// Ensure scheme is not configured twice.
		if scheme != "" {
			return "", fmt.Errorf("Scheme is configured explicitly (%s) and set within an address (%s)", scheme, address)
		}

		scheme = strings.SplitN(address, "://", 2)[0]
	}

	// If scheme is still empty, determine it based on the used protocol.
	if scheme == "" {
		scheme = "unix"
	...
	switch scheme {
	case "unix":
		// Ensure port is not configured for unix.
		if port != "" || url.Port() != "" {
			return "", fmt.Errorf("Port cannot be configured for unix address")
		}
	...
	case "https":
		...
		// Ensure port is not configured twice.
		if port != "" && url.Port() != "" {
			return "", fmt.Errorf("Port is configured explicitly (%s) and set within the address (%s)", port, address)
		}
		...
		// If port is still empty, determine it based on the protocol.
		if port == "" {
			port = "8443"
			if protocol == "simplestreams" {
				port = "443"

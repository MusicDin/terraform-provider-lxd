...
func TestAccStoragePool_cephfs(t *testing.T) {
	clusterName, cephfsPath, _ := acctest.PreCheckCeph(t)

	poolName := acctest.GenerateName(2, "-")
	driverName := "cephfs"
	poolConfig := map[string]string{
		"cephfs.cluster_name": clusterName,
		"cephfs.path":         cephfsPath,
	}
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr("lxd_storage_pool.storage_pool1", "config.cephfs.path", cephfsPath),
				),
			},
		},
	})

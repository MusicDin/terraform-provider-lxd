...
func TestAccProvider_trustTokenEnv(t *testing.T) {
	defer acctest.ResetLXDEnvVars()
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				// Ensure authentication succeeds if fully qualified address is
			...
			{
				// Ensure authentication succeeds if address is configured using
				// scheme, address, and port environment variables.
				PreConfig: func() {
					configDir = t.TempDir()
					os.Setenv("LXD_REMOTE", "tf-remote-token")
					os.Setenv("LXD_SCHEME", "https")
					os.Setenv("LXD_ADDR", "127.0.0.1")
					os.Setenv("LXD_PORT", "8443")
					os.Setenv("LXD_TOKEN", acctest.ConfigureTrustToken(t))
				},
				Config: testAccProvider_remoteServerEnv(configDir),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("lxd_noop.noop", "remote", "tf-remote-token"),
					resource.TestCheckResourceAttr("lxd_noop.noop", "project", "default"),
					resource.TestCheckResourceAttrSet("lxd_noop.noop", "server_version"),
				),
			},
...
func TestAccProvider_trustPasswordEnv(t *testing.T) {
	defer acctest.ResetLXDEnvVars()
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				PreConfig: func() {
					configDir = t.TempDir()
					os.Setenv("LXD_REMOTE", "tf-remote-pass")
					os.Setenv("LXD_SCHEME", "https")
					os.Setenv("LXD_ADDR", "127.0.0.1")
					os.Setenv("LXD_PORT", "8443")
					os.Setenv("LXD_PASSWORD", acctest.ConfigureTrustPassword(t))
				...
				Config: testAccProvider_remoteServerEnv(configDir),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("lxd_noop.noop", "remote", "tf-remote-pass"),
					resource.TestCheckResourceAttr("lxd_noop.noop", "project", "default"),
					resource.TestCheckResourceAttrSet("lxd_noop.noop", "server_version"),
				),
			},
